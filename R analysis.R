...
library(lme4)
library(lmerTest) #to get p values from "summary(lm_model)"
...
Rank_RT <- read_csv("1. Rank_RT.csv")
...
OL_MLD.Temp.plot <- 
  ...
  labs(x = "Temperature (℃) ", y="Mean lunar day (MLD)")+
  theme_classic()+
  theme(text=element_text(size=12,  family="Arial"))+
  theme(axis.title.y = element_text(margin = margin(r=10)),
        axis.title.x = element_text(margin = margin(t=10)))
...
#I6 is missing
#Why? Their  ACF values are too flat, erratic, or do not resemble a sine-like pattern
#____________________________________________________
...
#Data
RO_Temp <- read_csv("C:/Users/lamkw/Downloads/3. R test/Nano Project (Louis Laptop)/2. RO_Temp.csv")
...
TLE_RO.plot <- 
  ...
  geom_smooth()+
  scale_color_manual(values = c("OL" = "pink", "IL" = "skyblue"))+
...
png("Fig4.png", units="mm", width=150, height=200, res=1200)
Fig4 <-
  ggplot(fitted_results, aes(x = x)) +
  geom_point(aes(y = y)) +
  geom_line(aes(y = fit), color = "blue") +
